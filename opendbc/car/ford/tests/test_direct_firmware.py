...
class FirmwareBus:
  ...
  def __init__(self, frame=ABS_FRAME, bus=0, address=0x768):
  ...
  def send(self, messages):
    ...
    for m in messages:
      if m.src == 0 and m.address == 0x760 and m.dat[:4] == b'\x03\x22\xf1\x88' and self.frame is not None:
...
def run_query(request, bus):
  return IsoTpParallelQuery(bus.send, bus.recv, request.bus, [0x760], request.request,
...
class TestDirectFirmware(unittest.TestCase):
  ...
  def test_query_scope(self):
    ...
    self.assertEqual(set(r.whitelist_ecus), {CarParams.Ecu.abs, CarParams.Ecu.eps})
