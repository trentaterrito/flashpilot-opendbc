...
from opendbc.car.ford.values import DBC, RADAR, FordFlags
...
class RadarInterface(RadarInterfaceBase):
  def __init__(self, CP):
    ...
    self.dropout_shadow = SteerAssistDropoutShadow() if CP.flags & FordFlags.STEER_ASSIST_RADAR_SHADOW else None

    self.updated_messages = set()
    self.radar = RADAR.STEER_ASSIST_DATA if CP.flags & FordFlags.STEER_ASSIST_RADAR else DBC[CP.carFingerprint].get(Bus.radar)
  ...
  def update(self, can_strings):
    ...
    if can_strings:
      self.current_time = max(packet[0] for packet in can_strings) * 1e-9
  ...
  def _update_steer_assist(self):
    ...
    if self.dropout_shadow is not None:
      decision = self.dropout_shadow.update(self.current_time, int(confidence),
    ...
    self.pts[0].deprecated.aRel = float('nan')
    self.pts[0].deprecated.yvRel = msg["CmbbObjRelLat_V_Actl"]
    self.pts[0].deprecated.measured = True
