...
class TestFlashPilotFordPathAngleSafety(unittest.TestCase):
  ...
  def test_release_history_is_cleared_after_rejection_or_reset(self):
    # 'revoke' (V1's AOL host-initiated lateral-revocation protocol,
    # safety_lateral_revoke()) is intentionally not exercised here -- V2-4 does
    # not implement AOL/MADS, and there is no non-AOL equivalent of "host revocation"
    # to preserve. wrong_bus/wrong_length/relay (outer safety_tx_hook() rejections)
    # and reset/inactive (ford_tx_hook's own existing inline reset) remain covered.
    for sign in (-1, 1):
      for action in ('shadow', 'wrong_bus', 'wrong_length', 'relay', 'inactive', 'mode_off'):
        with self.subTest(sign=sign, action=action):
          self._prepare_opposite_path_history(sign)
          if action == 'shadow':
